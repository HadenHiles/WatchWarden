  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white">Jellyseerr</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            WatchWarden surfaces curated streaming content to Jellyseerr&apos;s Discover tab so your
            users can request it. Sliders only surface titles; roster automation is configured
            separately.
          </p>
        </div>
        <button
        ...
        <div className="space-y-6">
          {Object.entries(grouped)
            ...
            .map(([provider, entries]) => (
              <div
                key={provider}
                className="rounded-xl bg-gray-900 border border-gray-800/80 overflow-hidden"
              >
                <div className="px-4 py-3 border-b border-gray-800/60 flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600">
                    {entries.length} slider{entries.length !== 1 ? "s" : ""}
                  </span>
                </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                          {slider.jellyseerrSliderId ? (
                            ...
                            <span className="text-[11px] text-yellow-600">Pending first sync</span>
                          )}
                          {slider.itemCount > 0 && (
      ...
      <div className="rounded-xl bg-gray-900/50 border border-gray-800/40 p-4">
        <p className="text-xs text-gray-600 leading-relaxed">
          <span className="text-gray-400 font-medium">How it works:</span> WatchWarden tracks
          trending titles on each streaming platform and creates managed sliders in
          Jellyseerr&apos;s Discover tab. Your household users browse Jellyseerr, see
          WatchWarden&apos;s curated content, and request what they want — you review and approve in
          Jellyseerr as normal. Creating a slider does not submit requests.
        </p>
      </div>
    </div>
  );
