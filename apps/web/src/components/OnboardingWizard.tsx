...
import { CheckCircle, ChevronRight, ChevronLeft, Loader2, Wifi, WifiOff, Clapperboard } from "lucide-react";
...
const CRON_LABELS: [keyof SchedulesForm, string][] = [
    ...
    ["librarySyncCron", "Library Sync (Jellyseerr)"],
...
export function OnboardingWizard() {
    ...
    async function handleTestPlex() {
        setPlexTest({ state: "testing", message: "" });
        const result = await testConnection("plex", plex.baseUrl, plex.token);
        setPlexTest({ state: result.success ? "ok" : "fail", message: result.message ?? "" });
    }
                ...
                <div className="rounded-2xl border border-gray-800/60 bg-gray-900/70 p-8 shadow-2xl backdrop-blur-sm">
                    ...
                    {currentStep === "welcome" && (
                        <>
                            ...
                            <h1 className="text-2xl font-bold text-white mb-2 text-center">Welcome to Watch Warden</h1>
                            <p className="text-gray-400 mb-6 text-sm leading-relaxed text-center">
                                Watch Warden scores trending titles against your household&#39;s watch history,
                                then automatically keeps your Plex collections fresh — no Kometa config files required.
                            </p>
                            <ul className="space-y-2.5 mb-6">
                                {[
                                    ...
                                    <li key={label} className="flex items-start gap-3 text-sm text-gray-300">
                                        <ChevronRight className={`w-4 h-4 mt-0.5 flex-shrink-0 ${highlight ? "text-brand-400" : "text-gray-600"}`} />
                                        <span>
                                            <span className={`font-medium ${highlight ? "text-brand-300" : "text-white"}`}>{label}</span> — {desc}
                                        </span>
                                    </li>
                                ))}
                            </ul>
                    ...
                    {currentStep === "plex" && (
                        <>
                            ...
                            <div className="space-y-4">
                                <Field label="Server URL" hint="e.g. http://192.168.8.3:32400">
                                    <input
                                        type="url"
                                        value={plex.baseUrl}
                                        onChange={(e) => {
                                            setPlex((f) => ({ ...f, baseUrl: e.target.value }));
                                            setPlexTest({ state: null, message: "" });
                                        }}
                                        placeholder="http://192.168.8.3:32400"
                                        className={INPUT_CLS}
                                    />
                                </Field>
                                <Field
                                    ...
                                    hint={
                                        <>
                                            ...
                                            or via Plex Web → Settings → Troubleshooting → XML
                                        </>
                                    }
                                >
                                    <input
                                        ...
                                        value={plex.token}
                                        onChange={(e) => {
                                            setPlex((f) => ({ ...f, token: e.target.value }));
                                            setPlexTest({ state: null, message: "" });
                                        }}
                                ...
                                <TestButton
                                    state={plexTest.state}
                                    message={plexTest.message}
                                    disabled={!plex.baseUrl || !plex.token}
                                    onTest={handleTestPlex}
                                />
                            </div>
                        </>
                    )}
                            ...
                            <div className="space-y-3">
                                {CRON_LABELS.map(([key, label]) => (
                                    <div key={key} className="flex items-center gap-3">
                                        <label className="text-sm text-gray-300 w-48 flex-shrink-0">{label}</label>
                                        <input
                                            type="text"
                                            value={schedules[key]}
                                            onChange={(e) =>
