...
function candidateScore(candidate: Candidate) {
    ...
    return score > 0.005 ? Math.round(score * 100) : null;
}

function candidateSortStrength(candidate: Candidate) {
    const score = candidateScore(candidate);
    if (score != null) return 10_000 + score;
    const ranked = bestProviderSignal(candidate);
    if (ranked?.providerRank != null) return Math.max(0, 1_000 - ranked.providerRank);
    return Math.round(Math.max(0, ...candidate.trendSignals.map((signal) => signal.trendScore * 100)));
}

function candidateSignalLabel(candidate: Candidate) {
    const score = candidateScore(candidate);
    if (score != null) return `${score}/100 recommendation score`;
    const ranked = bestProviderSignal(candidate);
    if (ranked?.providerRank != null) return `#${ranked.providerRank} live platform rank`;
    return "Current shelf match";
}
...
export function PlexCurationQueue({ shelves, onChanged }: { shelves: Shelf[]; onChanged: () => void }) {
    ...
    const candidates = useMemo(() => {
        ...
        return [...merged.values()].filter((candidate) => !dismissedIds.has(candidate.id)).sort((a, b) =>
            candidateSortStrength(b) - candidateSortStrength(a) || b.shelves.length - a.shelves.length || a.title.localeCompare(b.title));
                ...
                {isLoading ? <div className="text-center"><Loader2 className="mx-auto h-7 w-7 animate-spin text-brand-400"/><p className="mt-3 text-xs text-gray-500">Building your review deck…</p></div> : error ? <p className="text-sm text-red-400">{error.message}</p> : !current ? <div className="max-w-sm text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-500/10"><CheckCheck className="h-7 w-7 text-emerald-400"/></div><h3 className="mt-4 text-xl font-semibold text-white">Queue cleared</h3><p className="mt-2 text-sm leading-relaxed text-gray-500">Nothing else matches these filters. Automations will keep discovering fresh candidates.</p></div> : <div className="w-full max-w-3xl">
                    <div
                        ...
                        <div className="relative grid gap-5 p-5 sm:grid-cols-[180px_minmax(0,1fr)] sm:p-6">
                            ...
                            <div className="min-w-0"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-white/8 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-gray-300">{current.mediaType === "MOVIE" ? "Movie" : "Series"}</span>{current.streamingOn.slice(0, 3).map((provider) => <span key={provider} className="rounded-full bg-purple-500/10 px-2.5 py-1 text-[10px] text-purple-300">{provider}</span>)}</div><h3 className="mt-3 text-2xl font-bold tracking-tight text-white">{current.title}</h3><p className="mt-1 text-sm text-gray-500">{current.year ?? "Year unknown"} · {candidateSignalLabel(current)}</p><p className="mt-4 line-clamp-4 text-sm leading-6 text-gray-300">{current.overview || "No synopsis is available yet."}</p>
