...
const FILTER_DESCRIPTIONS: Record<string, string> = {
    ACTIVE_TRENDING: "Titles currently in ACTIVE_TRENDING status — the hottest content right now",
...
export default function PlexCollectionsPage() {
    ...
    async function handleCreate() {
        ...
        try {
            ...
            setShowForm(false);
            setForm({ name: "", sectionId: "", mediaType: "MOVIE", filter: "ACTIVE_TRENDING" });
            await mutate();
        } finally {
            setCreating(false);
        }
    ...
    async function handleTriggerSync() {
        ...
        try {
            ...
            // Small delay then trigger collection sync
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                            WatchWarden-managed collections synced directly to your Plex server
                        </p>
                    </div>
                </div>
            ...
            {/* How it works footer */}
            <div className="rounded-xl border border-gray-800/40 p-4 text-xs text-gray-500 space-y-1.5 leading-relaxed">
                <p className="font-medium text-gray-400">How collections work</p>
                <p>
                    WatchWarden syncs each enabled collection to Plex on a schedule (default every 6h 45min).
                    Items are added/removed automatically as titles move in and out of the selected filter.
                </p>
                <p>
                    Use <span className="text-gray-300">Sync Now</span> above to trigger an immediate library scan + collection update.
                    Collections are created in Plex the first time they sync and will appear on the home screen of all users.
                </p>
            </div>
        </div>
    );
